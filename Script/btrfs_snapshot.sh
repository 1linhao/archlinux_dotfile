...
mount_disk() {
    if mountpoint -q "$MOUNT_POINT"; then
        ...
        if ! sudo umount "$MOUNT_POINT"; then
            echo "Unmount failed, operation aborted"
            exit 1
        fi
    fi

    if ! sudo mount /dev/disk/by-uuid/"$DISK_UUID" "$MOUNT_POINT" --mkdir 2>/dev/null; then
        echo "Unable to mount disk, UUID check: $DISK_UUID"
        exit 1
    fi
    MOUNTED=1
    cd "$MOUNT_POINT" || {
...
init_snapshot_json() {
    ...
    if [ ! -f "$SNAPSHOT_JSON" ]; then
        ...
        if ! jq -e '.snapshots | arrays' "$SNAPSHOT_JSON" >/dev/null 2>&1; then
            echo "Fixing invalid snapshot.json..."
            echo '{"snapshots": []}' | sudo tee "$SNAPSHOT_JSON" > /dev/null
        fi
    }
}
...
delete_snapshots() {
    ...
    if ! jq -e '.snapshots | arrays' "$SNAPSHOT_JSON" >/dev/null 2>&1; then
        echo "snapshot.json is invalid or malformed"
        return 1
    fi
...
check_arch_live() {
    if [ ! -f /etc/arch-release ] || ! grep -q "Arch Linux" /etc/os-release || [ ! -d /run/archiso ]; then
        echo "Must be in Arch Linux Live environment to perform restore"
        exit 1
    fi
}
...
print_help() {
    ...
    echo ""
    echo "Commands:"
    echo "  create [commit]   Create a new snapshot with optional commit message"
    echo "  list              List all snapshots"
    echo "  delete <id...>    Delete specified snapshot IDs (multiple allowed)"
    echo "  restore <id>      Restore specified snapshot ID (requires Arch Live environment)"
    echo ""
}
